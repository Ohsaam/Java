...
public class RandomGame{
	...
	//메소드 파라미터 자리에 리턴타입이 존재하는 메소드를 호출할 수 있다. -  보여주자
	//public void messagePrint(int user) {//user = 5복사됨 - call by value
		//System.out.println(account(user));//24-> 5를 쥐고 간다.-> 높여라 출력
	//}
	//42-43(dap=-1)-44(채번하는 메소드 호출한다)-14-15-16(dap변수에 새로운숫자대입)
	//49-36
	//
	public static void main(String[] args) {
		RandomGame rg = new RandomGame();//F6
		rg.newGame();//이 메소드가 호출되어야 0부터 9사이의 숫자가 채번된다. 그래서 채번된 숫자는 dap에 담김
		//messagePrint호출할 때 파라미터 자리에 5 를 넘겨주고36번에서 account()호출될 때 지역변수에 복사된 user값
		//즉 5가 파라미터로 넘어간다.
		//24번에서도 user값은 5가 된다.
		//rg.account(5);
		Scanner sc = new Scanner(System.in);//java.util.Scanner
		for(int i=0;i<5;i++) {
			System.out.println("0~9사이의 숫자를 입력하세요.(입력하면 엔터치기)");
			int user = sc.nextInt();//초기화가 5번 반복된다 - 덮어쓰기가 된다. - 맨 마지막에 덮어씌워진 값만 유지 된다.
			String hint = rg.account(user);//메소드를 따라가서 추적할땐 F5누른다.			
