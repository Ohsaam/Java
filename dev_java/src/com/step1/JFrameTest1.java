...
public class JFrameTest1 {
	//선언부
	//자바가 제공하는 클래스를 호출하려면 우린 인스턴스화 해야함
	//아래 문자을 작성하면 jf라는 변수로 그 클래스가 소유(정의된)하고 있는
	//전역변수나 메소드를 호출할 수 있다. - 사용이라고 함
	JFrame jf = new JFrame();
	//생성자 선언
	//코딩을 한다는 건 JVM과 소통하는 것이다. - 시나리오
	//사용자 정의 메소드 선언- 화면에 윈도우창을 하나 띄워본다
	public void initDisplay() {
		//메소드 호출은 끝에 세미콜론을 붙인다
		//setTitle메소드의 파라미터 자리에는 문자열 타입이 와야 함
		//이미 타입이 결정되어 있다.
		jf.setTitle("처음 만들어보는 윈도우창");
		jf.setSize(500, 400);
		jf.setVisible(true);
		jf.setVisible(false);
		jf.setVisible(true);
...
클래스 이름에 빨간줄은 JVM이 해당 클래스이름을 찾을 수 없을때 표시
