 ...
 * 전역변수 - 인스턴스변수.전역변수만 가능하고
 * 지역변수 - 인스턴스변수.지역변수 호출은 불가함(불법- 컴파일안됨)
 * 정적변수 - 클래스타입.변수명으로 사용함(싱글톤.하나다, 공유된다)
 */
public class SONATA {//접근제한자  class예약어사용함-클래스 선언 클래스이름을 지어준다 - 대문자로 시작하라-낙타표기법 - 외계인 코드(좋지않아요)
	...
	public static void main(String[] args) {
		...
		//지역변수는 불가하다.
		SONATA myCar = null;//선언만 했다. 아직 생성하지 않았다. 아직(변수 myCar를 ) 사용할 수 없다. 타입만 결정되었다.
