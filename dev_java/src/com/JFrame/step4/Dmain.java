...
// 사이드이펙트-부수효과
class D{
	//하나로 합치니까 C에 대한 인스턴스화가 필요없게 되었다. - 이러한 이유로 원본하나가 관리됨
	//그래서 0이 출력되는게 아니고 240이 출력된다.
	//총점을 담는 변수선언
	int tot;
	//평균을 담는 변수선언
	int avg;
	void hap(int n1, int n2, int n3) {
		//이전 소스에서는 클래스를 세개로 나누었다. -> 두개로 통합하였다.
		//c.tot =>c가 필요없는건 전역변수가 내안에 있기 때문이다.
		//이것만으로 복사본이아니라 원본을 유지(웹:쿠키와 세션 - 완결편)할 수  있다.
		tot = n1 + n2 + n3;
	}
	//평균을 구하는 메소드 선언
	void average() {
